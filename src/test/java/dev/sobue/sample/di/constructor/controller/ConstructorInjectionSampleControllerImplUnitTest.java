...
class ConstructorInjectionSampleControllerImplUnitTest {
  ...
  class Get {
    ...
    void returnsServiceResult(String input) {
      // setup
      when(constructorInjectionSampleService.get(input)).thenReturn(input);

      // when
      String result = controller.get(input);

      // then
      verify(constructorInjectionSampleService).get(input);
